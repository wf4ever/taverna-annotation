...
/**
 * Annotation health checker
 * 
 */
public class AnnotationActivityHealthChecker implements
		HealthChecker<AnnotationActivity> {

	public boolean canVisit(Object o) {
		// Return True if we can visit the object. We could do
		// deeper (but not time consuming) checks here, for instance
		// if the health checker only deals with AnnotationActivity where
		// a certain configuration option is enabled.
		return o instanceof AnnotationActivity;
	}
	...
	public VisitReport visit(AnnotationActivity activity, List<Object> ancestry) {
		AnnotationActivityConfigurationBean config = activity.getConfiguration();
		...
		return new VisitReport(HealthCheck.getInstance(), activity,
				"Annotation service OK", HealthCheck.NO_PROBLEM, subReports);
	}
